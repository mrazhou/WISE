...
def main():
    ...
    if 'qwen2_5' in args.reasoning_model_path.lower():
        ...
        reasoning_model = Qwen2VLForConditionalGeneration.from_pretrained(
            args.reasoning_model_path,
            torch_dtype=torch.bfloat16,
            attn_implementation="flash_attention_2",
            device_map="auto",
    ...
    QUESTION_TEMPLATE = \
        ...
        "Output the detailed thinking process in <d_think> </d_think>, the streamlined thinking process in <think> </think> and final answer in <answer> </answer> tags." \
        "Output the one bbox and points of two largest inscribed circles inside the interested object in JSON format." \
        "i.e., <d_think> detailed thinking process here </d_think>" \
        "<think> streamlined thinking process here </think>" \
    ...
    QUESTION_TEMPLATE = \
        "Please find '{Question}' with bbox and points." \
        "Compare the difference between objects and find the most closely matched one." \
        "Output the streamlined thinking process in <think> </think> and final answer in <answer> </answer> tags." \
        "Mentally perform and skip output the detailed thinking process in <d_think> </d_think>. " \
        "Output the one bbox and points of two largest inscribed circles inside the interested object in JSON format." \
        "i.e., think> streamlined thinking process here </think>" \
        "<answer>{Answer}</answer>"
    ...
    if 'shorter' in args.output_path:
        # shorter
        QUESTION_TEMPLATE = \
            "Please find '{Question}' with bbox and points." \
            "Compare the difference between objects and find the most closely matched one." \
            "Output the streamlined thinking process in <think> </think> and final answer in <answer> </answer> tags." \
            "Output the one bbox and points of two largest inscribed circles inside the interested object in JSON format." \
            "i.e., <think> streamlined thinking process here (within 10 words, the shorter the better, one word is best) </think>" \
            "<answer>{Answer}</answer>"

    if 'd_think-infer' in args.output_path:    
        # shorter
        QUESTION_TEMPLATE = \
            "Please find '{Question}' with bbox and points." \
            "Compare the difference between objects and find the most closely matched one." \
            ...
            "i.e., <think> streamlined thinking process here </think>" \
            "<answer>{Answer}</answer>" \
            "<d_think> detailed thinking process here </d_think>"
    
    if 'no_think' in args.output_path:
        # no think
        QUESTION_TEMPLATE = \
            "Please find '{Question}' with bbox and points." \
            "Compare the difference between objects and find the most closely matched one." \
    ...
    if 'no_c_think' in args.output_path:
        # no think
        QUESTION_TEMPLATE = \
            "Please find '{Question}' with bbox and points." \
            "Compare the difference between objects and find the most closely matched one." \
            "Output the detailed thinking process in <d_think> </d_think> and the final answer in <answer> </answer> tags." \
            "Output the one bbox and points of two largest inscribed circles inside the interested object in JSON format." \
            "<answer>{Answer}</answer>" \
            "<d_think> detailed thinking process here </d_think>"

    if 'one_sentence' in args.output_path:
        # one_sentence
        QUESTION_TEMPLATE = \
            "Please find '{Question}' with bbox and points." \
            "Compare the difference between objects and find the most closely matched one." \
            "Output the streamlined thinking process in <think> </think> and final answer in <answer> </answer> tags." \
            "Output the one bbox and points of two largest inscribed circles inside the interested object in JSON format." \
            "i.e., <think> streamlined thinking process here (one sentence) </think>" \
            "<answer>{Answer}</answer>"

    if 'raw_prompt' in args.output_path:
        # raw_prompt
        QUESTION_TEMPLATE = "{Question}. Output the bounding box coordinate in the same JSON."
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            for id_idx in range(len(batch_output_text)):
                ...
                try:
                    ...
                    _mask_output_path = ""
                    if iou < 0.1:
                        _mask_output_path = mask_output_path.replace("infer_mask", "infer_mask_bad")
                    elif iou > 0.9:
                        _mask_output_path = mask_output_path.replace("infer_mask", "infer_mask_good")
                    
                    if '_good' in _mask_output_path or '_bad' in _mask_output_path:
                        visualize_image_and_masks_v1_enhanced(
                            batch_id_list[id_idx]["image"],
                            mask,
                            gt_mask,
                            output_path=f"{_mask_output_path}/{batch_id_list[id_idx]['image_id']}.png",
                            bbox = bbox,
